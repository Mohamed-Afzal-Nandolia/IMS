...
export default function PurchaseOrdersPage() {
  ...
  const { data, isLoading } = useInvoices({ type: 'purchase_order', search, page });
  ...
  const handleDelete = async (id: string) => {
    try { await deleteInvoice.mutateAsync(id); addToast({ type: 'success', title: 'Purchase Order Deleted' }); setDeleteConfirm(null); }
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      <motion.div variants={item} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">Purchase Orders</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{totalItems} orders</p>
        </div>
        <button onClick={() => setShowModal(true)} className="px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 transition-all flex items-center gap-2 self-start">
          <LuPlus className="w-4 h-4" /> New Order
        </button>
      </motion.div>
      ...
      <motion.div variants={item} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Total Orders', value: formatCurrency(totalAmount), color: 'text-indigo-600' },
          { label: 'Paid Amounts', value: formatCurrency(totalPaid), color: 'text-emerald-600' },
          { label: 'Pending Balance', value: formatCurrency(totalAmount - totalPaid), color: 'text-amber-600' },
        ].map((s) => (
      ...
      {/* Filters */}
      <motion.div variants={item} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white dark:bg-gray-800 border border-gray-200/80 dark:border-gray-700/50">
        <LuSearch className="w-4 h-4 text-gray-400" />
        <input type="text" placeholder="Search PO number..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="bg-transparent text-sm outline-none w-full text-gray-700 dark:text-gray-300" />
      </motion.div>
      ...
      <motion.div variants={item} className="bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200/80 dark:border-gray-700/50 overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-gray-400 mt-1">Create your first purchase order</p>
            <button onClick={() => setShowModal(true)} className="mt-4 px-5 py-2 rounded-xl bg-indigo-600 text-white text-sm font-semibold"><LuPlus className="w-4 h-4 inline mr-1" /> New Order</button>
          </div>
        ) : (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setViewInvoice(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-gray-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">PO {viewInvoice.invoiceNumber}</h2>
              ...
              <div className="space-y-3 text-sm">
                {[['Supplier', viewInvoice.party?.name || '—'], ['Date', new Date(viewInvoice.issueDate).toLocaleDateString('en-IN')],
                  ...
                  ['Total Amount', formatCurrency(viewInvoice.totalAmount)], ['Paid', formatCurrency(viewInvoice.amountPaid || 0)],
        ...
        {deleteConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setDeleteConfirm(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-gray-800 rounded-2xl max-w-sm w-full p-6 text-center">
              ...
              <p className="text-sm text-gray-500 mt-2">This will permanently delete this purchase order.</p>
      ...
      {/* New Invoice Modal */}
      <AnimatePresence>
        {showModal && <InvoiceFormModal invoiceType="purchase_order" onClose={() => setShowModal(false)} />}
      </AnimatePresence>
    </motion.div>
  );
}
      ...
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-gray-800 rounded-2xl max-w-4xl w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Ordered Items</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 border-t border-gray-200 dark:border-gray-700 pt-4">
          <div>
            ...
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Terms, delivery instructions..." rows={3} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm outline-none resize-none" />
          </div>
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-base font-bold border-t pt-2 mt-2"><span>Total</span><span className="text-indigo-600 tabnum">{formatCurrency(grandTotal)}</span></div>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 pt-6 mt-4 border-t border-gray-100 dark:border-gray-700">
          ...
          <button onClick={handleSubmit} disabled={createInvoice.isPending} className="px-6 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2">
            {createInvoice.isPending && <LuLoader className="w-4 h-4 animate-spin" />} Create Order
          </button>
        </div>
      </motion.div>
    </motion.div>
