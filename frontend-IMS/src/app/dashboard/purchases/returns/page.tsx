...
export default function PurchaseReturnsPage() {
  ...
  const { data, isLoading } = useInvoices({ type: 'purchase_return', search });
  const invoices = data?.invoices || [];

  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      <motion.div variants={item}><h1 className="text-2xl font-bold text-gray-900 dark:text-white">Purchase Returns</h1><p className="text-sm text-gray-500 mt-1">{invoices.length} debit notes</p></motion.div>
      ...
      <motion.div variants={item} className="bg-white dark:bg-gray-800/60 rounded-2xl border border-gray-200/80 dark:border-gray-700/50 overflow-hidden">
        ...
        : invoices.length === 0 ? <div className="text-center py-20"><LuFileText className="w-12 h-12 mx-auto text-gray-300 mb-3" /><p className="text-gray-500 font-medium">No purchase returns</p><p className="text-sm text-gray-400 mt-1">Debit notes will appear here</p></div>
        : <table className="w-full text-sm"><thead><tr className="text-left text-gray-500 border-b bg-gray-50/50"><th className="px-5 py-3">Return #</th><th className="px-5 py-3">Supplier</th><th className="px-5 py-3">Date</th><th className="px-5 py-3 text-right">Amount</th></tr></thead>
