    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={(e) => e.stopPropagation()} className="bg-white dark:bg-gray-800 rounded-2xl max-w-4xl w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">New Sales Return</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 border-t border-gray-200 dark:border-gray-700 pt-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Return Reason / Notes</label>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="E.g., damaged goods, wrong item..." rows={3} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm outline-none resize-none" />
          </div>
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-base font-bold border-t pt-2 mt-2"><span>Total Refund</span><span className="text-indigo-600 tabnum">{formatCurrency(grandTotal)}</span></div>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 pt-6 mt-4 border-t border-gray-100 dark:border-gray-700">
          ...
          <button onClick={handleSubmit} disabled={createInvoice.isPending} className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 text-white text-sm font-semibold disabled:opacity-50 flex items-center gap-2">
            {createInvoice.isPending && <LuLoader className="w-4 h-4 animate-spin" />} Issue Return
          </button>
        </div>
      </motion.div>
    </motion.div>
